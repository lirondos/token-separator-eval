...
for metric,model in dev.items():
    with pd.ExcelWriter("/home/ealvarezmellado/lrec2024/token-separator-eval/dev.xlsx", engine="openpyxl", mode="a") as writer:
        pd.Dataframe.from_dict(mydict).to_excel(writer, sheet_name=metric)
        
for metric,model in test.items():
    with pd.ExcelWriter("/home/ealvarezmellado/lrec2024/token-separator-eval/test.xlsx", engine="openpyxl", mode="a") as writer:
        pd.Dataframe.from_dict(mydict).to_excel(writer, sheet_name=metric)
